...
| [`LEDGER-0005`](LEDGER-0005-gitlab-as-source-of-truth/) | GitLab CE full source-of-truth (Phase 1 shipped: https://git.yousirjuan.ai) | `phase-1-shipped` | 2026-05-20 | _pending_ | Phase 1: live; Phase 2 + 3 TODO |
| [`LEDGER-0006`](LEDGER-0006-gitlab-warm-standby/) | Historical GitLab/Tailscale warm-standby design; superseded by DGX Gitea + WireGuard | `superseded` | 2026-05-20 | 2026-07-22 | Historical evidence only; do not replay |
...
| [`LEDGER-0016`](LEDGER-0016-uptime-kuma-monitoring/) | Uptime Kuma — external uptime monitoring + multi-channel alerts (Telegram / email / Pushover) | `superseded-by-0017` | 2026-05-21 | _pending_ | `sudo bash ledger/LEDGER-0016-uptime-kuma-monitoring/playbooks/install.sh install` (on VPS) — subdomain approach replaced by LEDGER-0017 path-based |
...
| [`LEDGER-0018`](LEDGER-0018-tailscale-to-wireguard-migration/) | Historical migration plan; current permanent law is WireGuard-only on `10.1.0.0/24` | `superseded-by-live-wireguard` | 2026-05-21 | 2026-07-22 | Historical evidence only; use current fleet wiring |
...
| [`LEDGER-0026`](LEDGER-0026-vps-into-wg-mesh/) | Historical onboarding record; live mesh is `10.1.0.0/24` with VPS `.2` and DGX `.5` | `shipped-and-superseded-addressing` | 2026-05-29 | 2026-07-22 | Do not replay its `10.0.0.x` examples; use current inventory |
...
| [`LEDGER-0032`](LEDGER-0032-claude-ssh-user/) | `claude` SSH user + Members/ACL — shared AI automation account + support for adding human family Members with full ACL (sudo, AllowUsers, groups, key+password via CLAUDE_PASSWORD in local env). Includes add-member.sh wrapper, set-claude-password.sh helper, 07-add-member runbook. WireGuard-native (Tailscale banned). | `enhanced` (password + Members) | 2026-06-02 | 2026-06-09 | `docs/family-fleet-ssh-claude.md` + `bash .../playbooks/install-from-mac.sh` + `bash .../add-member.sh <name>` |
| [`LEDGER-0033`](LEDGER-0033-mail-jailynmarvin/) | Self-hosted mail — docker-mailserver on clinic-vps for jailynmarvin.com | `installed-dns-pending` | 2026-06-03 | _pending_ | VPS: `/opt/mail-jailynmarvin`; runbook `01-godaddy-dns-cutover.md` |
| [`LEDGER-0034`](LEDGER-0034-mail-vps-nas-hybrid/) | Mail hybrid — VPS live + NAS WG mount + hourly rsync | `blocked-on-nas-share` | 2026-06-03 | _pending_ | UGOS `01-nas-ugos-mail-share.md` then `install-nas-hybrid.sh` on VPS |
