...
**Execute, don't delegate (RL-0051 — Avery 2026-06-16):** If it does not violate an explicit user rule or require **sudo**, **just do it** — git, SSH, forge-sync, verify, cleanup. Never hand the operator paste blocks for safe work. Boss Moves only. Full rule: [`.claude/rules/agent-executes-dont-delegate.md`](.claude/rules/agent-executes-dont-delegate.md) · [`rules/GLOBAL-RULES-FOR-USING-NEPHEW.md`](rules/GLOBAL-RULES-FOR-USING-NEPHEW.md) Rule 1.
